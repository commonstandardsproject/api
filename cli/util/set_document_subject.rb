...
# $db[:new_standard_sets].find({_id: "5A99211C6D874994A2CFEE2AE40023DD_D2384125_grade-pre-k"}).each_with_index{|standards_set, index|
$db[:new_standard_sets].find().each_with_index{|standards_set, index|
  p "#{index}: Converting: #{standards_set["_id"]}"
  document = $db[:standards_documents].find({_id: standards_set["documentId"]}).to_a.first
  $db[:new_standard_sets].find({_id: standards_set["_id"]}).update_one({
    :$set => {
