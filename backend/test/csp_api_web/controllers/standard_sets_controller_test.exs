defmodule CspApiWeb.StandardSetsControllerTest do
  ...
  test "GET /standard_sets/:id returns the set with parent/ancestor ids filled in", %{conn: conn} do
    %{"data" => data} =
  ...
  test "GET /standard_sets/:id?standardsAsArray=true returns the standards as a list", %{conn: conn} do
    %{"data" => data} =
      conn
  ...
  test "unknown id returns 200 with an empty data object", %{conn: conn} do
    assert %{"data" => %{}} ==
             conn |> get("/api/v1/standard_sets/does-not-exist") |> json_response(200)
  end
end
