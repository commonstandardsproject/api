> **ARCHIVED — legacy Ruby/Sinatra backend.**
>
> This is the original Ruby implementation of the CSP API, kept here for
> posterity. It has been superseded by the Elixir/Phoenix port in
> [`../backend`](../backend) and is **not built, tested, or deployed** by
> this monorepo (CI and Northflank target `backend/` only). Nothing here
> is wired into the platform build. See the repo root
> [`README`](../README.md) for the current architecture.
>
> Preserved as of the pre-migration state; full history is in git.
